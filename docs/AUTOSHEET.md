...
3. **Contextual combat:** the roll/action-context contract now exists. A roll carries action kind, mode, weapon identity and tags, touch, full-attack membership, sequence index, maneuver and situational flags; an action is modeled as a standard attack, a multi-weapon full attack with explicit per-weapon members, or a maneuver. Covered by tests: one shared Haste extra across weapons, touch eligibility (Deadly Aim vs rays), Rapid Shot inside ranged non-touch full attacks, Power Attack handedness, maneuver-conditional CMB, natural secondary attacks and sight-based checks. Still authored rather than derived: combined two-weapon/natural/flurry sequence limits, proficiency, buckler hand use, cover and cover-dependent effects, denied Dexterity, jump-only penalties and action-economy restrictions. Do not concatenate single-weapon sequences into a combined full attack. Grants are reminders, not action enforcement. See `docs/open-decisions.md` for the maneuver, multi-source extra-attack and damage/drain questions this leaves open.
...
The roll-context contract from the previous frontier is now implemented, so the remaining design decision is a campaign rules profile (PF1e vs 3.5/3.PF and which optional combat options apply), followed by spellcasting/feature-resource models. This pass adds neither runtime LLM interpretation nor optimization/feat automation.
