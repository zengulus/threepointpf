...
Existing characters and `FeatureInstance` behavior remain compatible. `SpellDefinition` keeps spell semantics separate from abilities; `SpellcastingSource` owns casting mode, casting ability, list identity/access, progression rows, and source-specific known, spellbook, and prepared state. Progression rows determine caster level, maximum spell level, slots and optional spells-known allowances. Advancement content can add effective levels to a stable source ID across tracks. Caster level, concentration, source-and-level DC, and slot capacity are derived with inspectable contributions. Slot expenditure is persisted in ordinary `ResourceState` using generated source-owned resource identities; maximum capacities are never stored.

Spontaneous casts require list access, known-spell membership, an eligible level, and a remaining slot. Prepared casts additionally require an unexpended prepared allocation; each allocation has independent identity. Cast validation is atomic, and modeled damage returns an ordinary sourced roll plan. The Spells tab supports multiple sources, a compact source/progression editor, local spell authoring, known/spellbook selection, casting, preparation, and reset. A small curated spell set ships as rules data. Bonus slots are an authored extension point (`standard` or `none`); the conventional ability bonus table is not computed. Prestige content can advance a fixed source identity or bind advancement to an existing lifecycle choice among eligible source IDs. Automatic spell attack plans, healing execution, target saves, spell resistance, buffs, components, and action enforcement remain unsupported. The feature is a foundation, not complete spellcasting.
