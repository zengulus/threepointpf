...
import { progressionCatalog, progressionOptions } from "@threepointpf/rules-data";
import type { AdvancementSlot, CharacterInput, AbilityId, BonusType, Contribution, DefenseContext, DerivedProgressionFeature, Effect, EffectTargetId, EvaluationResult, FeatureInstance } from "@threepointpf/rules-schema";
...
function repository(): CharacterRepository {
  ...
  if (url && key) return new SupabaseCharacterRepository(createClient(url, key), progressionCatalog);
  return new InMemoryCharacterRepository(progressionCatalog);
}
...
function ContributionTree({ contribution, depth = 0 }: { contribution: Contribution; depth?: number }) {
  return <><div className="contribution" style={{ marginLeft: depth * 12 }}><span className="contribution-value">{formatModifier(contribution.value)}</span><span><b>{contribution.label}</b><em>{contribution.note ?? contribution.bonusType ?? "base / derived"}</em></span></div>{contribution.children?.map((child, index) => <ContributionTree key={`${child.source}-${index}`} contribution={child} depth={depth + 1} />)}</>;
}
...
function Breakdown({ selected }: { selected: { label: string; evaluation: { value: number; contributions: Contribution[] } } | null }) {
  ...
  return <div className="breakdown"><div className="eyebrow">AUDIT TRAIL</div><h3>{selected.label} <span>= {selected.evaluation.value}</span></h3><div className="contributions">{selected.evaluation.contributions.map((item, index) => <ContributionTree key={`${item.source}-${index}`} contribution={item} />)}</div></div>;
}
...
const catalogOptions = progressionOptions();
const firstProgressionId = catalogOptions[0]?.id ?? "";
...
function AdvancementEditor({ slots, onChange }: { slots?: AdvancementSlot[]; onChange: (slots: AdvancementSlot[]) => void }) {
  if (!slots?.length) {
    return <section className="panel advancement-panel"><div className="panel-heading"><div><span className="eyebrow">ADVANCEMENT</span><h2>Ordered progression tracks</h2></div><span className="helper">N-track / no gestalt flag</span></div><p className="advancement-copy">Start a structured advancement record to derive level, BAB, saves, hit dice and class-chart provenance from the catalog.</p><button className="button quiet" data-testid="advancement-start" onClick={() => onChange([{ id: "level-1", tracks: [{ id: "track-1", entry: { progressionId: firstProgressionId } }] }])}>Start advancement</button></section>;
  }
  const trackIds = slots[0]?.tracks.map((track) => track.id) ?? [];
  const replace = (next: AdvancementSlot[]) => onChange(next);
  const changeEntry = (slotId: string, trackId: string, progressionId: string) => replace(slots.map((slot) => slot.id !== slotId ? slot : { ...slot, tracks: slot.tracks.map((track) => track.id !== trackId ? track : { ...track, entry: { progressionId } }) }));
  const addLevel = () => {
    const previous = slots[slots.length - 1]!;
    const id = nextStableId("level", slots.map((slot) => slot.id));
    replace([...slots, { id, tracks: previous.tracks.map((track) => ({ id: track.id, entry: { progressionId: track.entry.progressionId } })) }]);
  };
  const removeLastLevel = () => { if (slots.length > 1) replace(slots.slice(0, -1)); };
  const addTrack = () => {
    const id = nextStableId("track", trackIds);
    replace(slots.map((slot) => ({ ...slot, tracks: [...slot.tracks, { id, entry: { progressionId: firstProgressionId } }] })));
  };
  const removeTrack = (trackId: string) => { if (trackIds.length > 1) replace(slots.map((slot) => ({ ...slot, tracks: slot.tracks.filter((track) => track.id !== trackId) }))); };
  return <section className="panel advancement-panel"><div className="panel-heading"><div><span className="eyebrow">ADVANCEMENT</span><h2>Ordered progression tracks</h2></div><span className="helper">{slots.length} levels · {trackIds.length} tracks</span></div><p className="advancement-copy">Rows are character levels; columns are complete tracks. Entries resolve in row then track order when the same class occurs more than once.</p><div className="advancement-grid-wrap"><table className="advancement-grid"><thead><tr><th>Level</th>{trackIds.map((trackId, index) => <th key={trackId}>Track {index + 1}<button className="table-action" aria-label={`Remove track ${index + 1}`} onClick={() => removeTrack(trackId)} disabled={trackIds.length === 1}>×</button></th>)}</tr></thead><tbody>{slots.map((slot, slotIndex) => <tr key={slot.id}><th>Level {slotIndex + 1}</th>{slot.tracks.map((track, trackIndex) => <td key={track.id}><select aria-label={`Level ${slotIndex + 1} track ${trackIndex + 1} progression`} value={track.entry.progressionId} onChange={(event) => changeEntry(slot.id, track.id, event.target.value)}>{catalogOptions.map((definition) => <option key={definition.id} value={definition.id}>{definition.name}</option>)}</select></td>)}</tr>)}</tbody></table></div><div className="advancement-actions"><button className="button quiet" data-testid="advancement-add-level" onClick={addLevel}>+ Level</button><button className="button quiet" data-testid="advancement-remove-level" onClick={removeLastLevel} disabled={slots.length === 1}>− Last level</button><button className="button quiet" data-testid="advancement-add-track" onClick={addTrack}>+ Track</button></div></section>;
}

function AdvancementSummary({ progressionLevels, features, onInspect }: { progressionLevels: Record<string, EvaluationResult>; features: DerivedProgressionFeature[]; onInspect: (label: string, evaluation: EvaluationResult) => void }) {
  const levels = Object.entries(progressionLevels);
  if (!levels.length) return null;
  return <section className="panel advancement-summary"><div className="panel-heading"><div><span className="eyebrow">DERIVED ADVANCEMENT</span><h2>Global progression levels</h2></div><span className="helper">Click for provenance</span></div><div className="progression-levels">{levels.map(([id, evaluation]) => {
    const definition = progressionCatalog[id];
    const label = definition?.name ?? id;
    return <button className="progression-level" data-testid={`progression-level-${id}`} key={id} onClick={() => onInspect(`${label} level`, evaluation)}><span>{label}</span><strong>Level {evaluation.value}</strong><small>inspect →</small></button>;
  })}</div>{features.length > 0 && <div className="progression-features"><span className="eyebrow">UNLOCKED METADATA</span>{features.map((feature) => <div key={`${feature.progressionId}-${feature.id}-${feature.slotId}-${feature.trackId}`}><b>{feature.name}</b><small>{feature.progressionId} {feature.level} · {feature.trackId}</small></div>)}</div>}</section>;
}
...
export function App() {
  ...
  const [newFeatureTarget, setNewFeatureTarget] = useState<EffectTargetId>("attack.melee");
  ...
  const engine = useMemo(() => new RulesEngine(character, { progressionCatalog }), [character]);
  ...
  const updateAdvancement = (advancementSlots: AdvancementSlot[]) => update({ advancementSlots, baseBab: undefined, baseSaves: undefined, hitDiceCount: undefined });
  ...
  return <main className="app-shell">
    ...
    <section className="hero"><div><span className="eyebrow">PATHFINDER 1E · VERTICAL SLICE</span><input className="character-name" aria-label="Character name" value={character.name} onChange={(event) => update({ name: event.target.value })} /><p>Authored inputs flow through features, contributions and reducers into an inspectable sheet.</p></div><div className="hero-meta"><span>CAMPAIGN</span><b>{character.campaignId ?? "Unassigned"}</b><span>LEVEL / BAB</span><b>{character.advancementSlots ? `${derived.advancement?.slotCount ?? 0} / ${formatModifier(derived.bab.value)}` : `— / ${formatModifier(derived.bab.value)}`}</b></div></section>
    <div className="layout"><div className="main-column">
      <section className="panel inputs-panel"><div className="panel-heading"><div><span className="eyebrow">AUTHORED INPUTS</span><h2>Core numbers</h2></div><span className="pill">recompute on edit</span></div><div className="ability-grid">{abilityOrder.map((id) => <div className="ability-input" key={id}><span>{id.toUpperCase()}</span><input aria-label={`${abilityNames[id]} base score`} type="number" value={character.baseAbilities[id]} onChange={(event) => updateAbility(id, event.target.value)} /><b>{formatModifier(derived.abilities[id].modifier.value)}</b></div>)}</div><div className="inline-fields">{!character.advancementSlots?.length && <><Field label="Base BAB" value={character.baseBab ?? ""} onChange={(value) => update({ baseBab: Number(value) || 0 })} /><Field label="Hit dice" value={character.hitDiceCount ?? ""} onChange={(value) => update({ hitDiceCount: Math.max(1, Number(value) || 1) })} /></>}<Field label="HP before CON" value={character.baseHpBeforeConstitution} onChange={(value) => update({ baseHpBeforeConstitution: Number(value) || 0 })} /><Field label="Damage taken" value={character.damageTaken} onChange={(value) => update({ damageTaken: Math.max(0, Number(value) || 0) })} /><Field label="Temporary HP" value={character.temporaryHp} onChange={(value) => update({ temporaryHp: Math.max(0, Number(value) || 0) })} /><Field label="Land speed" value={character.baseLandSpeed ?? 30} onChange={(value) => update({ baseLandSpeed: Number(value) || 0 })} /></div>{!character.advancementSlots?.length && <div className="save-inputs">{(["fortitude", "reflex", "will"] as const).map((id) => <Field key={id} label={`Base ${id}`} value={character.baseSaves?.[id] ?? ""} onChange={(value) => update({ baseSaves: { ...(character.baseSaves ?? { fortitude: 0, reflex: 0, will: 0 }), [id]: Number(value) || 0 } })} />)}</div>}</section>
      <AdvancementEditor slots={character.advancementSlots} onChange={updateAdvancement} />
      {derived.advancement && <AdvancementSummary progressionLevels={derived.advancement.progressionLevels} features={derived.advancement.features} onInspect={inspect} />}
      <section className="panel"><div className="panel-heading"><div><span className="eyebrow">DERIVED SHEET</span><h2>Defenses & combat</h2></div><span className="helper">Click any value to inspect</span></div><div className="stats-grid"><StatCard label="Base Attack Bonus" value={formatModifier(derived.bab.value)} evaluation={derived.bab} onClick={() => inspect("Base Attack Bonus", derived.bab)} testId="stat-bab" /><StatCard label="Armor Class" value={derived.ac.value} evaluation={derived.ac} onClick={() => inspect("Armor Class", derived.ac)} testId="stat-ac" /><StatCard label="Touch AC" value={derived.touchAc.value} evaluation={derived.touchAc} onClick={() => inspect("Touch AC", derived.touchAc)} testId="stat-touch-ac" /><StatCard label="Flat-footed" value={derived.flatFootedAc.value} evaluation={derived.flatFootedAc} onClick={() => inspect("Flat-footed AC", derived.flatFootedAc)} testId="stat-flat-footed" /><StatCard label="HP" value={`${derived.currentHp} / ${derived.maxHp.value}`} evaluation={derived.maxHp} onClick={() => inspect("Maximum HP", derived.maxHp)} testId="stat-current-hp" /><StatCard label="Max HP" value={derived.maxHp.value} evaluation={derived.maxHp} onClick={() => inspect("Maximum HP", derived.maxHp)} testId="stat-max-hp" /><StatCard label="Initiative" value={formatModifier(derived.initiative.value)} evaluation={derived.initiative} onClick={() => inspect("Initiative", derived.initiative)} testId="stat-initiative" /><StatCard label="CMB" value={formatModifier(derived.cmb.value)} evaluation={derived.cmb} onClick={() => inspect("CMB", derived.cmb)} testId="stat-cmb" /><StatCard label="CMD" value={derived.cmd.value} evaluation={derived.cmd} onClick={() => inspect("CMD", derived.cmd)} testId="stat-cmd" /></div><div className="saves-row">{(["fortitude", "reflex", "will"] as const).map((id) => <div className="roll-row" key={id}><button className="value-link" onClick={() => inspect(id, derived.saves[id])}><span>{id}</span><strong>{formatModifier(derived.saves[id].value)}</strong></button><button className="roll-button" data-testid={`roll-${id}`} onClick={() => roll(`${id} save`, () => engine.createSaveRollPlan(id))}>ROLL d20</button></div>)}</div></section>
