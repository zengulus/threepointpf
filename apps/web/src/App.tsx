...
const demoCharacter: CharacterInput = {
  ...
  features: [
    ...
    { id: "rage", name: "Rage", description: "A combat toggle affecting the whole dependency graph.", enabled: false, effects: [{ kind: "modifier", target: "ability.str", value: 4, bonusType: "morale" }, { kind: "modifier", target: "ability.con", value: 4, bonusType: "morale" }, { kind: "modifier", target: "save.will", value: 2, bonusType: "morale" }, { kind: "modifier", target: "ac", value: -2, bonusType: "untyped" }] },
...
export function App() {
  ...
  const [newFeatureKind, setNewFeatureKind] = useState<"modifier" | "set">("modifier");
  ...
  function addFeature() {
    ...
    const appliesTo: DefenseContext[] | undefined = newFeatureDefenseScope === "all" || newFeatureTarget !== "ac" ? undefined : newFeatureDefenseScope === "normal" ? ["normal"] : ["normal", "touch"];
    const effect: Effect = newFeatureKind === "set" ? { kind: "set", target: newFeatureTarget, value: Number(newFeatureValue) || 0 } : { kind: "modifier", target: newFeatureTarget, value: Number(newFeatureValue) || 0, bonusType: newFeatureType, appliesTo };
  ...
  return <main className="app-shell">
    ...
    <section className="hero"><div><span className="eyebrow">PATHFINDER 1E · VERTICAL SLICE</span><input className="character-name" aria-label="Character name" value={character.name} onChange={(event) => update({ name: event.target.value })} /><p>Authored inputs flow through features, contributions and reducers into an inspectable sheet.</p></div><div className="hero-meta"><span>CAMPAIGN</span><b>{character.campaignId ?? "Unassigned"}</b><span>LEVEL / BAB</span><b>— / {formatModifier(character.baseBab)}</b></div></section>
    <div className="layout"><div className="main-column">
      <section className="panel inputs-panel"><div className="panel-heading"><div><span className="eyebrow">AUTHORED INPUTS</span><h2>Core numbers</h2></div><span className="pill">recompute on edit</span></div><div className="ability-grid">{abilityOrder.map((id) => <div className="ability-input" key={id}><span>{id.toUpperCase()}</span><input aria-label={`${abilityNames[id]} base score`} type="number" value={character.baseAbilities[id]} onChange={(event) => updateAbility(id, event.target.value)} /><b>{formatModifier(derived.abilities[id].modifier.value)}</b></div>)}</div><div className="inline-fields"><Field label="Base BAB" value={character.baseBab} onChange={(value) => update({ baseBab: Number(value) || 0 })} /><Field label="HP before CON" value={character.baseHpBeforeConstitution} onChange={(value) => update({ baseHpBeforeConstitution: Number(value) || 0 })} /><Field label="Current HP" value={character.currentHp} onChange={(value) => update({ currentHp: Number(value) || 0 })} /><Field label="Land speed" value={character.baseLandSpeed ?? 30} onChange={(value) => update({ baseLandSpeed: Number(value) || 0 })} /></div><div className="save-inputs">{(["fortitude", "reflex", "will"] as const).map((id) => <Field key={id} label={`Base ${id}`} value={character.baseSaves[id]} onChange={(value) => update({ baseSaves: { ...character.baseSaves, [id]: Number(value) || 0 } })} />)}</div></section>
      <section className="panel"><div className="panel-heading"><div><span className="eyebrow">DERIVED SHEET</span><h2>Defenses & combat</h2></div><span className="helper">Click any value to inspect</span></div><div className="stats-grid"><StatCard label="Armor Class" value={derived.ac.value} evaluation={derived.ac} onClick={() => inspect("Armor Class", derived.ac)} testId="stat-ac" /><StatCard label="Touch AC" value={derived.touchAc.value} evaluation={derived.touchAc} onClick={() => inspect("Touch AC", derived.touchAc)} testId="stat-touch-ac" /><StatCard label="Flat-footed" value={derived.flatFootedAc.value} evaluation={derived.flatFootedAc} onClick={() => inspect("Flat-footed AC", derived.flatFootedAc)} testId="stat-flat-footed" /><StatCard label="Max HP" value={derived.maxHp.value} evaluation={derived.maxHp} onClick={() => inspect("Maximum HP", derived.maxHp)} testId="stat-max-hp" /><StatCard label="Initiative" value={formatModifier(derived.initiative.value)} evaluation={derived.initiative} onClick={() => inspect("Initiative", derived.initiative)} testId="stat-initiative" /><StatCard label="CMB" value={formatModifier(derived.cmb.value)} evaluation={derived.cmb} onClick={() => inspect("CMB", derived.cmb)} testId="stat-cmb" /><StatCard label="CMD" value={derived.cmd.value} evaluation={derived.cmd} onClick={() => inspect("CMD", derived.cmd)} testId="stat-cmd" /></div><div className="saves-row">{(["fortitude", "reflex", "will"] as const).map((id) => <div className="roll-row" key={id}><button className="value-link" onClick={() => inspect(id, derived.saves[id])}><span>{id}</span><strong>{formatModifier(derived.saves[id].value)}</strong></button><button className="roll-button" data-testid={`roll-${id}`} onClick={() => roll(`${id} save`, () => engine.createSaveRollPlan(id))}>ROLL d20</button></div>)}</div></section>
    ...
    </div><aside className="side-column"><Breakdown selected={selected} /><section className="panel features-panel"><div className="panel-heading"><div><span className="eyebrow">FEATURES / EFFECTS</span><h2>Combat toggles</h2></div></div>{character.features.map((feature) => <div className={`feature-row ${feature.enabled ? "enabled" : ""}`} key={feature.id}><button className="toggle" aria-label={`Toggle ${feature.name}`} aria-pressed={feature.enabled} onClick={() => toggleFeature(feature.id)}><span /></button><div><b>{feature.name}</b><small>{feature.effects.map((effect) => effect.kind === "modifier" ? `${effect.value >= 0 ? "+" : ""}${effect.value} ${effect.bonusType} → ${effect.target}` : effect.kind).join(" · ")}</small></div><i>{feature.enabled ? "ON" : "OFF"}</i></div>)}<div className="feature-form"><input placeholder="Feature name" aria-label="New feature name" value={newFeatureName} onChange={(event) => setNewFeatureName(event.target.value)} /><select aria-label="New feature kind" value={newFeatureKind} onChange={(event) => setNewFeatureKind(event.target.value as "modifier" | "set")}><option value="modifier">Modifier</option><option value="set">Replacement / set</option></select><select aria-label="New feature target" value={newFeatureTarget} onChange={(event) => setNewFeatureTarget(event.target.value as TargetId)}><option value="attack.melee">Melee attack</option><option value="damage.melee">Melee damage</option><option value="ac">AC</option><option value="initiative">Initiative</option><option value="save.fortitude">Fortitude</option><option value="save.reflex">Reflex</option><option value="save.will">Will</option><option value="ability.str">Strength</option><option value="ability.dex">Dexterity</option><option value="skill.all">All skills</option></select><div className="mini-fields"><input aria-label="New feature value" type="number" value={newFeatureValue} onChange={(event) => setNewFeatureValue(event.target.value)} /><select aria-label="New feature bonus type" value={newFeatureType} onChange={(event) => setNewFeatureType(event.target.value as BonusType)}>{bonusTypeOptions.map((type) => <option key={type}>{type}</option>)}</select></div>{newFeatureTarget === "ac" && <select aria-label="New feature AC applicability" value={newFeatureDefenseScope} onChange={(event) => setNewFeatureDefenseScope(event.target.value as "all" | "normal" | "normalTouch")}><option value="all">AC: all contexts</option><option value="normal">AC: normal only</option><option value="normalTouch">AC: normal + touch</option></select>}<button className="button quiet" onClick={addFeature}>+ Add structured effect</button></div></section><div className="side-note"><span className="eyebrow">SOURCE OF TRUTH</span><p>Only authored inputs, features and attacks are persisted. The sheet always derives its current values from the pure TypeScript engine.</p></div></aside></div>
  </main>;
}
