...
import type { CharacterInput, AbilityId, BonusType, Contribution, DerivedCharacter, Effect, FeatureInstance, TargetId } from "@threepointpf/rules-schema";
...
export function App() {
  ...
  function addFeature() {
    ...
    const effect: Effect = { kind: "modifier", target: newFeatureTarget, value: Number(newFeatureValue) || 0, bonusType: newFeatureType };
  ...
  function addAttack() {
    ...
    update({ attacks: [...character.attacks, { id: `attack-${Date.now()}`, name: newAttackName.trim(), attackAbility: newAttackAbility, damageAbility: newAttackAbility, damageAbilityMultiplier: 1, baseDamage: { count: Number(match[1]), sides: Number(match[2]) }, mode: "melee", attackTags: ["melee"] }] }); setNewAttackName(""); setNotice("Added attack");
  }
