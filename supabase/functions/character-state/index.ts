...
Deno.serve(async (request) => {
  ...
  try {
    ...
    return json({ name: character.name, currentHp: derived.currentHp, maxHp: derived.maxHp.value, damageTaken: derived.damageTaken, temporaryHp: derived.temporaryHp, bab: derived.bab.value, ac: derived.ac.value, saves: { fortitude: derived.saves.fortitude.value, reflex: derived.saves.reflex.value, will: derived.saves.will.value }, attacks: derived.attacks.map((attack) => ({ id: attack.definition.id, name: attack.definition.name, modifier: attack.attack.value, fullAttack: attack.fullAttack.map((strike) => strike.value) })) });
